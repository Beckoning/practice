package com.pay.payment.dubboconsumer.center.api.cluster;
 ...
 */
public abstract class MyAbstractClusterInvoker<T> implements Invoker<T> {

	private static final Logger logger = LoggerFactory.getLogger(MyAbstractClusterInvoker.class);
	...
	public MyAbstractClusterInvoker(Directory<T> directory) {
		this(directory, directory.getUrl());
	}

	public MyAbstractClusterInvoker(Directory<T> directory, URL url) {
		if (directory == null) {
	 ...
	 * previously selected list, or, if this invoker is unavailable, then continue
	 * step b (reselect), otherwise return the first selected invoker</br>
	 * <p>
	 * b) Reselection, the validation rule for reselection: selected > available.
	 * This rule guarantees that the selected invoker has the minimum chance to be
	 * one in the previously selected list, and also guarantees this invoker is
	 * available.
	 *
	 * @param loadbalance
	 *            load balance policy
		...
		if ((selected != null && selected.contains(invoker))
			...
			try {
				...
				if (rinvoker != null) {
					...
					// Check the index of current selected invoker, if it's not the last one, choose
					// the one at index+1.
	...
	public Result invoke(final Invocation invocation) throws RpcException {
		...
		// return doInvoke(invocation, map());
	...
	protected abstract Result doInvoke(Invocation invocation, Map<String, Invoker<T>> invokerMap) throws RpcException;
	...
	protected Map<String, Invoker<T>> map() throws RpcException {
		return ((RegistryDirectory) directory).getUrlInvokerMap();
	}

	/**
	 * Init LoadBalance.
	 * <p>
	 * if invokers is not empty, init from the first invoke's url and invocation if
	 * invokes is empty, init a default LoadBalance(RandomLoadBalance)
	 * </p>
	 *
	 * @param invokers
	 *            invokers
	...
	protected LoadBalance initLoadBalance(List<Invoker<T>> invokers, Invocation invocation) {
		if (CollectionUtils.isNotEmpty(invokers)) {
			return ExtensionLoader.getExtensionLoader(LoadBalance.class)
					.getExtension(invokers.get(0).getUrl().getMethodParameter(RpcUtils.getMethodName(invocation),
